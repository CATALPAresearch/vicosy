...
var IceServersHandler = window.IceServersHandler;
// var stream = window.stream;

export default function RTCPeerConnection5(options) {
  // reusable stuff
  ...
  console.debug(
    "rtc-configuration",
    JSON.stringify(rtcConfiguration, null, "\t")
  );
  ...
  if (options.attachStreams && options.attachStreams.length) {
    // mh: added s to options.attachStream[s].length
      ...
      if (options.onRemoteStream) {
        const waitComplete = setInterval(() => {
          ...
          if (
            ...
            options.onRemoteStream(peer.remoteMediaStream);
  ...
  var otherAudio = options.otherMediaConfig.audio;
  var otherVideo = options.otherMediaConfig.video;

  // var ownAudio = options.mediaConfig.audio;
  // var ownVideo = options.mediaConfig.video;

  // offer to receive: what do i want to receive from the other peer
  // constraints: if not correct, ice will fail
  // FF & chrome issue: Chrome requires { mandatory: constraints, optional: [] }
  // FF requires constraints without wrapper
  // and: "o"fferToReceive... in FF "O"fferToReceive... in Chrome -.-

  // o and O seems to be different in chrome and FF
  // on = constraint for offerer
  var constraints = {
    offerToReceiveAudio: otherAudio,
    offerToReceiveVideo: otherVideo,
  ...
  // on = constraint for offerer
  var constraintsChrome = {
    OfferToReceiveAudio: otherAudio,
    OfferToReceiveVideo: otherVideo
  };

  // var constraints = {
  //   offerToReceiveAudio: options.mediaConfig.audio,
  //   offerToReceiveVideo: options.mediaConfig.video
  // };

  // var constraints = {
  //   offerToReceiveAudio: options.otherMediaConfig.audio,
  //   offerToReceiveVideo: options.otherMediaConfig.video
  // };

  // console.info("OFFER CONSTRAINTS", constraints);
